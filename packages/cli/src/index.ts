...
program
  ...
  .action(() => {
    ...
    if (existsSync(workflowPath)) {
      ...
      writeFileSync(workflowPath, buildWorkflowTemplate(pm));
      console.log(
        `Created .github/workflows/sentinel.yml for ${pm} (locator-cache persistence, report artifact, PR summary comment).`,
      );
    }
  });
...
program
  ...
  .action((dir: string, opts: { dryRun?: boolean }) => {
    ...
    console.log(
      'Note: the codemod recognizes the standard `page` fixture name for goto/click/fill shorthands — an aliased fixture (e.g. `{ page: p }`) and option-bearing calls (`click({ force: true })`, `.not` assertions) are intentionally left for manual migration.',
    );
  });
